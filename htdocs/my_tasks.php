...
<body>
    ...
    <div class="wrapper">
        <h2>My Tasks</h2>
        <form name='search_bar2' action='all_tasks.php' method='POST' >
        <li>Search For Tasks With Description</li>
        <li><input type='text' name='search_bar2' value='' /></li>
        <li><input type='submit' name='search2' /></li>
        </form>
        <?php
        session_start();
        $userid = $_SESSION['user'];

        echo '<h5>' . 'Tasks you have created' . '</h5>';
        ob_start();
        include 'login.php';
        ob_end_clean();

        $db     = pg_connect("host=localhost port=5432 dbname=postgres user=postgres password=test");
        $result = pg_query($db, "SELECT t.description, t.due_date, t.due_time FROM tasks t, users u WHERE t.owner_id = u.user_id AND t.owner_id = $userid ORDER BY t.due_date");
        ...
        if (isset($_POST['accept'])) {
            ...
            $result = pg_query($db, "SELECT t.task_id FROM tasks t, users u WHERE t.owner_id = u.user_id AND t.owner_id = $userid ORDER BY t.due_date LIMIT 1 OFFSET $rownumber;");
